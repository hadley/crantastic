...
class diff_match_patch:
  ...
  def diff_toDelta(self, diffs):
    ...
    for (op, data) in diffs:
      if op == self.DIFF_DELETE:
        ...
        assert False, "Invalid diff operation in diff_toDelta()"
    return "".join(txt)
  ...
  def diff_fromDelta(self, text1, delta):
    ...
    Returns:
      Array of diff tuples or None if text1 is wrong length.
    """
    diffs = []
    ...
    for token in tokens:
      ...
      if token[0] == "-" or token[0] == "=":
        ...
        assert not token, "Invalid diff operation in diff_fromDelta()"
  ...
  def __str__(self):
    ...
    for (op, data) in self.diffs:
      if op == diff_match_patch.DIFF_DELETE:
        ...
        assert False, "Invalid diff operation in patch_obj.__str__()"
      txt.append(urllib.quote(data.encode("utf-8"), " =;',/~!@#$&*()+:?") + "\n")
    return "".join(txt)
